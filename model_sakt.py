...
class SAKT(nn.Module):
    ...
    problem_num=0,is_pmodel=False ,qa_binary=False,dropout=0.1, emb_dropout=0.1):
        ...
        if problem_num > 0 and is_pmodel:
            ...
            self.pa_emb = nn.Embedding(problem_num * 2 + 1, embed_dim)
        ...
        self.multiiheadattn = MultiHeadAttention(embed_dim, nhead, d_k, d_v, dropout, False)
        # self.activation1 = nn.ReLU()
        self.pos_enc = PositionalEmbedding(embed_dim)
        self.theta = nn.Parameter(torch.rand(1))
        self.out_linear = nn.Linear(embed_dim + embed_dim, 1)
    ...
    def forward(self, q, qa, problem, Q, test=False):
        ...
        if self.problem_num > 0 and self.is_pmodel:
            ...
            problem_answer = problem + problem * (qa//(self.skill_num+1))
            pa_emb = self.pa_emb(problem_answer)
        ...
        #qa_emb = self.pos_enc(qa_emb)
        t_out, _ = self.multiiheadattn(q = q_emb, k = qa_emb, v=qa_emb, current=0)
        ...
        t_out = torch.cat([torch.zeros((t_out.shape[0],1,t_out.shape[2])).to(device), t_out[:,:-1,:]], 1)
        y = torch.cat([q_emb, t_out], -1)
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        return x + self.weight[:, :x.size(1)].clone().detach()  # ( 1,seq,  Feature)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output):
        ...
        dec_output, dec_enc_attn = self.enc_attn(
            dec_output, enc_output, enc_output, 1)
